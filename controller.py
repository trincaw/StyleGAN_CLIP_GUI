...
class Controller:
    ...
    def set_default_paths(self):
        ...
        self.generator.set_network_pkl(path)
        self.generator.set_output_dir(output_path)
        self.view.network_path.setText(path)
        self.view.output_path.setText(output_path)

    def generate_image(self):
        network_pkl = self.view.network_path.text()
        ...
        translate_x = self.validate_int(
            self.view.translate_x_edit.text(), default=0)
        translate_y = self.validate_int(
            self.view.translate_y_edit.text(), default=0)
        rotate = self.validate_int(self.view.rotate_edit.text(), default=0)
        class_idx = self.validate_int(self.view.class_edit.text(), default=0)
        try:
